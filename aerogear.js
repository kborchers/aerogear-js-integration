/*! AeroGear JavaScript Library - v1.0.0 - 2013-05-23
...
AeroGear.Pipeline.adapters.Rest = function( pipeName, settings ) {
    ...
    // Privileged Methods
    /**
        Gets the currently associated authenticator module
        @private
        @augments Rest
        @returns {Object} The auth module
     */
    ...
    this.addAuthIdentifier = function( settings ) {
        return authenticator && authenticator.type === "Rest" ? authenticator.addAuthIdentifier( settings ) : settings;
    };
...
AeroGear.Pipeline.adapters.Rest.prototype.read = function( options ) {
    ...
    error = function( jqXHR, textStatus, errorThrown ) {
        var authenticator = that.getAuthenticator();
        if ( authenticator && authenticator.type === "BasicDigest" && jqXHR.status === 401 ) {
            // Begin basic or digest auth process
            authenticator.login( jqXHR, url, "GET", "read", that, options );
        } else {
...
/**
    ADD DESCRIPTION
 */
AeroGear.Auth.adapters.BasicDigest = function( moduleName, settings ) {
    // Allow instantiation without using new
    if ( !( this instanceof AeroGear.Auth.adapters.BasicDigest ) ) {
        return new AeroGear.Auth.adapters.BasicDigest( moduleName, settings );
    }

    settings = settings || {};

    // Private Instance vars
    var type = "BasicDigest",
        name = moduleName,
        authCallback = settings.authCallback;

    /**
     */
    this.getAuthCallback = function() {
        return authCallback;
    };
};

// Public Methods
AeroGear.Auth.adapters.BasicDigest.prototype.login = function( jqXHR, uri, httpMethod, method, pipe, options ) {
    var header = jqXHR.getResponseHeader("WWW-Authenticate");

    options.headers = options.headers || {};

    // Auth generaton methods
    function processBasicAuth( username, password ) {
        var credentials = Base64.encode( username + ":" + password );
    }

    function processDigestAuth( username, password ) {
        var currentHeader, digest,
            headers = {},
            headerAr = header.split(","),
            cnonce = new Date().getTime();
        for ( var item in headerAr ) {
            currentHeader = headerAr[ item ].split("=");
            if ( currentHeader[ 0 ].indexOf("realm") ) {
                headers.realm = currentHeader[ 1 ];
            } else {
                headers[ currentHeader[ 0 ] ] = currentHeader[ 1 ];
            }
        }

        digest = md5( username + ":" + headers.realm + ":" + password );
        digest += digest + ":" + headers.nonce + ":" + "00000001" + ":" + cnonce + ":" + headers.qop + ":";
        digest += md5( httpMethod + ":" + uri );
        digest = md5( digest );

        options.headers.Authorization = 'Digest , username="' + username + '", uri="' + uri + '", response="' + digest + '", nc="00000001", cnonce="' + cnonce + '"';
        pipe.read( options );
    }

    if ( header && header.indexOf("Basic") ) {
        this.getAuthCallback().call( processBasicAuth );
    } else if ( header && header.indexOf("Digest") ) {
        this.getAuthCallback().call( processDigestAuth );
    } else {
        throw "Unsupported Authentication Request";
    }
};
})( this );
